> A rendezvous, reprised; our...

# Radiant Revival

---

**Radiant Revival** is a work-in-progress visual overhaul mod for Terraria 1.4.5.

## Features

- [x] Main Menu Celestial Body Physics
  - [x] Velocity
  - [x] Right-Click Pull
- [ ] Title Screen Logo Lighting
  - [x] tModLoader
> <img width="616" height="225" alt="image" src="https://github.com/user-attachments/assets/03b61816-07cb-458c-91b2-025c2a2e8e10" />
  - [ ] Vanilla
  - [ ] Modded Support
- [x] Godrays
> <img width="640" height="317" alt="image" src="https://github.com/user-attachments/assets/f27e2a6e-a927-4b9c-b349-45577eb99f03" />
- [x] Revised Star Rendering
> <img width="640" height="360" alt="image" src="https://github.com/user-attachments/assets/f6042c69-bbcc-4c9d-9e14-5d661ed23f40" />
  - [x] 3D Starbox
  - [ ] Supernovae
- [ ] 3D Moon Rendering
- [x] Revised Meteor Shower/Shooting Stars Visuals
- [ ] Smooth Lighting
  - [ ] Tiles/Walls
  - [x] NPCs
  - [x] Projectiles
  - [x] Underground Backgrounds
> <img width="453" height="165" alt="image" src="https://github.com/user-attachments/assets/30acca55-7700-4101-a8e3-e067d55e2569" />
- [x] Revised Rain Visuals
  - [x] Tile Reflections
> <img width="397" height="275" alt="image" src="https://github.com/user-attachments/assets/3336bc6a-2875-4950-9304-810ab096f6c8" />
  - [x] Rainbows
- [x] Pitch Black Night Sky
- [x] Revised Aurora Borealis
- [ ] Background Lightning Visuals
- [ ] Revised Lunar Invasion Visuals
- [ ] Revised Water Visuals
  - [ ] Water Reflections
- [ ] Revised Shimmer Visuals
- [x] Atmosphere Visual
- [ ] Revised Nature/Treetop Lighting
- [x] Retro Lighting Fixes Circa [ink-zoey/RetroLightingFix](https://github.com/ink-zoey/FixRetroLighting)

## References

Previous implementations of some of the above features have existed in:

- [ink-zoey/ZensSky](https://github.com/ink-zoey/ZensSky) (AKA ZenSkies):
  - First iteration of the visual mod concept (although some features—namely star rendering—date back to older private mods,) later evolved into `sky-projects`;
- [gold-meridian/sky-projects](https://github.com/gold-meridian/sky-projects):
  - Second iteration, eventually spiritually merged into Radiant Revival in lieu of 1.4.5's sky changes and a desire to also make lighting changes.

Inspiration for lighting changes have been sourced from the most popular tModLoader lighting mods:

- [Fancy Lighting](https://steamcommunity.com/sharedfiles/filedetails/?id=2822950837):
  - Our personal favorite in terms of pre-existing mods, the initial feature parity goal;
- [Lights And Shadows](https://steamcommunity.com/sharedfiles/filedetails/?id=2670628346):
  - Primitive godrays implementation as a post-processing filter, has many issues, mostly a parity goal.
- [Dynamic Lights](https://steamcommunity.com/sharedfiles/filedetails/?id=2679594765):
  - Main inspiration for well-defined directional lighting with tile geometry shadow casting.
 
Inspiration for other visuals, sourced from various tModLoader mods:

- [Realistic Sky](https://steamcommunity.com/sharedfiles/filedetails/?id=3172916251)
  - Main inspiration for star rendering, as well as a possible baseline for atmosphere rendering.

The rough branding for the project is obviously inspired by [Radiant Revival feat. Hatsune Miku](https://www.youtube.com/watch?v=qHU3hf1b8NY) by Jamie Paige.

### TODO:
- Update with references to **Render Reprise** following [feat/render-reprise](https://github.com/gold-meridian/radiant-revival/tree/feat/render-reprise) being merged.
